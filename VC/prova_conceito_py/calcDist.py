...
def findPiece(img):
    ...
    cv2.drawContours(img, [workpiece], -1, (0, 255, 0), 1)
...
def findRelativeWorkpiecePosition(param_workpiece, ref):
    ...
    for i in range(len(param_workpiece)):
        x_pos.append(param_workpiece[i][0][0])
        y_pos.append(param_workpiece[i][0][1])
    ...
    referencePoint = ref[3]

    # TODO: - is not defined for a list. This might be doing something different than we want. To Investigate.
    relPosition = leftMost - referencePoint
