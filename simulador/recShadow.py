...
def local_minima(a):
    a = np.hstack((a[0], a, a[-1]))
    positions = np.r_[True, a[1:] < a[:-1]] & np.r_[a[:-1] < a[1:], True]
...
ax_recshadow = create_subplot(
    fig, 3 if same_plot else 6, title='Reconstrução')
ax_recshadow.set_ylim(-2, 2)
...
# r_points += np.random.normal(0, 0.1, r_points.shape)
...
shadow_recx = np.zeros(FRAMES)
shadow_recy = np.zeros(FRAMES)
...
def animate(i):
    ...
    x_came = np.max(y[y_zero]) + noise*np.random.normal(0, 0.01, 1)
    x_shadow = np.max(y) + noise*np.random.normal(0, 0.01, 1)
    ...
    shadow_recx = np.roll(shadow_recx, 1)
    shadow_recy = np.roll(shadow_recy, 1)

    deltashadow = (x_shadow-lastshadow)/dtheta

    shadow_recx[int(FRAMES/2)] = x_shadow * \
        np.cos(theta) - deltashadow*np.sin(theta)
    shadow_recy[int(FRAMES/2)] = x_shadow * \
        np.sin(theta) + deltashadow*np.cos(theta)
    ...
    locations = local_maxima(shadow_timeplot)
    print(locations)
    pts = shadow_timeplot[locations]
    print(pts)

    rec_came.set_data(np.linspace(0, FRAMES*dtheta, FRAMES)[locations], pts)
    rec_shadow.set_data(shadow_recx, shadow_recy)
