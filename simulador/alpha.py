...
def create_subplot(fig, i, xlim=(-3, 3), ylim=(-4, 4), **kwargs):
    ax = fig.add_subplot((130 + i) if same_plot else (230 + i),
                         adjustable='box',
                         aspect=1,
                         xlim=xlim, ylim=ylim,
                         **kwargs)
...
def animate(i):
    ...
    x_shadow = x[np.argmax(alpha)]
    y_shadow = y[np.argmax(alpha)]

    # Equação da reta: f(x) = m*x + b
    # m = deltaY/deltaX
    # x = (y-y0)/m + x0

    m = (y_shadow -  10)/(x_shadow)
    ...
    came_timeplot[int(FRAMES/2)] = x_shadow

    x_shadow = x[x>0][np.argmin(alpha[x>0])]
    y_shadow = y[x>0][np.argmin(alpha[x>0])]
